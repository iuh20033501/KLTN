...
const AddExamScreen = ({ navigation, route }: { navigation: any; route: any }) => {
    ...
    const [assignmentType, setAssignmentType] = useState<string>('CK'); // Giá trị mặc định là "CK"
    ...
    const validateInputs = () => {
        for (const question of assignments) {
            ...
            for (const key in question.answers) {
                if (!question.answers[key as keyof Exam['answers']].noiDung.trim()) {
    ...
    const handleInputChange = (index: number, field: string, value: string) => {
        ...
        if (field.startsWith('answer')) {
            const option = field as keyof Exam['answers'];
            newAssignments[index].answers[option].noiDung = value;
        } else {
            newAssignments[index][field as keyof Exam] = value as any;
        }
    ...
    const handleCorrectAnswerChange = (index: number, correctAnswer: string) => {
        ...
        Object.keys(newAssignments[index].answers).forEach((key) => {
            newAssignments[index].answers[key as keyof Exam['answers']].ketQua = key === `answer${correctAnswer}`;
        });
        setAssignments(newAssignments);
    };
    ...
    const handleSubmitAssignments = async () => {
        ...
        try {
            ...
            const idTest = response.data.idTest;
            console.log('Response from create baitest:', response.data.idTest);
                ...
                assignments.map(async (question) => {
                    ...
                    await Promise.all(
                        Object.keys(question.answers).map((key) => {
                            const answer = question.answers[key as keyof Exam['answers']];
                            const answerData = {
    ...
    return (
        <ImageBackground source={require('../../../image/bglogin.png')} style={styles.background}>
            <View style={styles.overlayContainer}>
                ...
                <Text style={styles.title}>Thêm Bài Thi</Text>

                <Text style={styles.label}>Tên Bài Thi</Text>
                <View style={{ width: '100%' }}>
                    <Picker
                        ...
                        style={styles.input}
                    >
                        <Picker.Item label="Bài Giữa Kì" value="GK" />
                        <Picker.Item label="Bài Cuối Kì" value="CK" />
                    </Picker>
                    <TextInput
                        style={styles.input}
                            ...
                            {['A', 'B', 'C', 'D'].map((option) => (
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 5, marginHorizontal: 10 }} key={option}>
                                    ...
                                    <TextInput
                                        ...
                                        value={question.answers[`answer${option}` as keyof Exam['answers']].noiDung}
