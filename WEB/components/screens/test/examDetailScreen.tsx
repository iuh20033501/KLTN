...
const ExamDetailScreen = ({ navigation, route }: { navigation: any, route: any }) => {
    const { idTest, trangThai } = route.params;
        ...
        try {
            ...
            const updatedQuestions = await Promise.all(
                questionsData.map(async (question: Question) => {
                    ...
                    return { ...question, dapAn: answers, linkAnh: question.linkAnh || null };
                })
            );
    ...
    const fetchAnswersForQuestion = async (questionId: number) => {
        try {
            ...
            const response = await http.get(`baitest/getCauTraLoi/${questionId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const uploadImageToS3 = async (fileUri: string) => {
        try {
            ...
            const fileName = `imgCauHoi/question-image-idTest-${idTest}-${Date.now()}`;
    ...
    const uploadAudioToS3 = async (fileUri: string) => {
        try {
            const fileBlob = await fetch(fileUri).then(res => res.blob());
            const fileName = `audioCauHoi/question-audio-idTest-${idTest}-${Date.now()}`;
            const uploadedUrl = await uploadFileToS3(fileBlob, fileName);
            return uploadedUrl;
        } catch (error) {
            console.error("Error uploading image to S3:", error);
    ...
    const saveCorrectAnswer = async () => {
        ...
        try {
            ...
            await http.put(
                `baitest/updateCauHoi/${selectedQuestion.idCauHoi}/${idTest}`,
                {
    ...
    return (
        <ImageBackground source={require('../../../image/bglogin.png')} style={styles.background}>
            <View style={styles.overlayContainer}>
                ...
                <Text style={styles.title}>Chi tiết bài thi</Text>
