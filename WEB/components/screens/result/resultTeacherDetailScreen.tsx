...
const ResultTeacherDetailScreen = ({ navigation, route }: { navigation: any, route: any }) => {
    const { idLopHoc, tenLopHoc } = route.params;
    ...
    const [thoiGianHoanThanh, setThoiGianHoanThanh] = useState('');
                ...
                members.map(async (member) => {
                    try {
                        const response = await http.get(
                            `/baitap/getTienTrinhHVBT/${member.idUser}/${idBaiTap}`,
                            { headers: { Authorization: `Bearer ${token}` } }
                        );

                        if (!response.data || response.data.length === 0) {
                            console.warn(`No progress found for student ${member.hoTen}`);
                            return { idHocVien: member.idUser, hoTen: member.hoTen, score: 0 };
                        }

                        return {
                            idHocVien: member.idUser,
                            hoTen: member.hoTen,
                            score: response.data.cauDung * 10,
                        };
                    } catch (error) {
                        console.error(
                            `Failed to fetch score for student ${member.hoTen}:`,
                            error
                        );
                        return { idHocVien: member.idUser, hoTen: member.hoTen, score: 0 };
                    }
        ...
        try {
            ...
            const scoresData = await Promise.all(
                members.map(async (member) => {
                    try {
                        const response = await http.get(
                            `baitest/getKetQua/${idTest}/${member.idUser}`,
                            { headers: { Authorization: `Bearer ${token}` } }
                        );
                        ...
                        return {
                            ...
                            thoiGianHoanThanh: response.data.thoiGianHoanThanh || null,
                        };
                    } catch (error) {
                        console.error(
                            `Failed to fetch test score for student ${member.hoTen}:`,
                            error
                        );
                        return { idHocVien: member.idUser, hoTen: member.hoTen, score: 0 };
                    }
    ...
    <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
            <Text style={styles.sectionTitle}>
                {activeTab === 'ExerciseScores' ? `Điểm: ${selectedAssignment}` : `Kết quả: ${selectedAssignment}`}
            </Text>
            <ScrollView style={styles.contentContainer}>
                {scores.map((score) => (
                    <View key={score.idHocVien} style={styles.memberCard}>
                        <Text style={styles.memberName}>{score.hoTen}</Text>
                        <Text style={styles.memberEmail}>
                            {members.find((member) => member.idUser === score.idHocVien)?.email}
                        </Text>
                        <Text style={styles.memberScores}>Điểm: {score.score}</Text>
                        {activeTab === 'ExamScores' && (
                            <Text style={styles.memberEmail}>
                                Thời gian làm bài: {score.thoiGianHoanThanh}
                            </Text>
                        )}
                    </View>
                ))}
...
export default ResultTeacherDetailScreen;
