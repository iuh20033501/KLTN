...
export default function ScoreBoardScreen({ route }: { route: any }) {
    ...
    try {
      ...
      const assignmentData = await Promise.all(
        response.data.map(async (assignment: any) => {
          ...
          setTotalQ(totalQuestionsResponse.data.length *10);
      ...
      {isLoading ? (
        ...
        tests.map((test) => (
          <View key={test.idTest} style={styles.scoreItem}>
            <View style={styles.scoreDetail}>
              <Text style={styles.scoreLabel}>{test.loaiTest === 'CK' ? 'Cuối Kỳ' : 'Giữa Kỳ'}</Text>
              <CircularProgress
                value={test.diemTest}
                radius={50}
                duration={1000}
